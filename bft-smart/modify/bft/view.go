...
func (v *View) processProposal() Phase {
	...
	for !gotPrePrepare {
		...
		case <-v.abortChan:
			return ABORT
		case msg := <-v.incMsgs:
			fmt.Printf("zhf add code: process incoming message in processProposal\n")
			v.processMsg(msg.sender, msg.Message)
...
func (v *View) processPrepares() Phase {
	...
	for len(voterIDs) < v.Quorum-1 {
		...
		case vote := <-v.prepares.votes: // prepares.votes 代表的是准备投票
			...
			fmt.Printf("zhf add code: receive prepare, current voterIDs: %v\n", voterIDs)
		}
	}
...
func (v *View) processCommits(proposal *types.Proposal) ([]types.Signature, Phase) {
	...
	for len(signatures) < v.Quorum-1 {
		...
		case <-v.abortChan:
			return nil, ABORT
		case msg := <-v.incMsgs:
			fmt.Printf("zhf add code: process incoming message in processCommits\n")
			v.processMsg(msg.sender, msg.Message)
